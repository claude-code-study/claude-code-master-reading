...
**Skill 콘텐츠 라이프사이클** — skill이 "어떻게 컨텍스트에 들어왔다 나가는지":

```mermaid
flowchart TD
    S0["세션 시작"] --> D["모든 skill의 *description*만 컨텍스트에 로드<br/>(Claude가 '뭐가 있는지' 알도록)"]
    D --> INV{"호출됨?<br/>(사용자 /skill-name 또는 Claude 자동)"}
    INV -- 아니오 --> D
    INV -- 예 --> R["렌더링된 SKILL.md 본문이<br/>대화에 *단일 메시지*로 추가됨<br/>(!`cmd` 주입은 이때 이미 실행돼 결과로 치환)"]
    R --> L["세션 내내 그 메시지로 잔존<br/>(Claude는 이후 턴에서 파일을 다시 읽지 않음<br/>→ '일회성 단계'가 아니라 '상시 지침'으로 작성)"]
    L --> C{"자동 압축(compaction) 발생?"}
    C -- 아니오 --> L
    C -- 예 --> RA["각 skill의 *가장 최근* 호출만 재첨부<br/>(앞 5,000토큰까지, 재첨부분 합산 25,000토큰 예산을<br/>최근 호출부터 채움 → 오래된 skill은 잘릴 수 있음)"]
    RA --> L
```

- 평소엔 **description만** 항상 떠 있고, **본문은 호출될 때만** 통째로 로드 → 긴 참조 자료를 둬도 평소 비용은 거의 없음
- 호출 후 동작에 영향이 없어 보이면 보통 콘텐츠는 그대로 있고 모델이 다른 접근을 고른 것 — `description`/지침을 강화하거나, 결정론이 필요하면 hook으로
- 미리 로드된 skill을 가진 **서브에이전트는 다름**: 본문이 시작 시 통째로 주입됨
- description이 너무 많아 예산을 넘으면 적게 쓰는 skill의 설명부터 잘림 → `/doctor`로 확인, `skillListingBudgetFraction` 설정으로 예산 조정

> 📚 출처: [code.claude.com/docs/ko/skills](https://code.claude.com/docs/ko/skills) (Skill content lifecycle / 압축 후 생존)
