...
-----------------------------------------------------------------------------------------------------------------------------------
## 📚 공식 문서 참고: Claude Code Overview 
출처: https://code.claude.com/docs/ko/overview (context7으로 조회, 2026-05-20)

### Claude Code란? 
- Anthropic이 만든 **에이전트형(agentic) 코딩 도구**
- 단순히 "코드 추천"을 넘어, 코드베이스를 직접 읽고 / 파일을 수정하고 / 명령을 실행하고 / 개발 도구와 통합한다 
- 비유: 옆자리 주니어 개발자에게 "이거 해줘"라고 말하는 것에 가까움 (단, 토큰은 아낀다는 전제) 

### 접근 채널 (어디서 쓸 수 있나) 
1. **터미널 CLI**: `claude` 명령어로 실행 (가장 기본)
2. **IDE 확장**: VS Code, JetBrains 
3. **데스크톱 앱**: Mac / Windows
4. **웹**: claude.ai/code
5. **협업 도구 연동**: Slack, GitHub Actions, GitLab (CI/CD) 

### 시작하는 법 (책 3장과 연결되는 부분)
```bash
...
# 3. 처음 한 번은 /init 실행 -> 프로젝트 컨벤션을 학습시켜 CLAUDE.md 자동 생성 
/init
```

### 워크플로의 본질적 변화 (Best Practices에서)
- **기존**: 코드를 내가 직접 쓰고 → 리뷰 요청 
- **Claude Code**: **원하는 결과(desired outcome)**를 설명하면 → Claude가 직접 구현 
- 즉, "어떻게(How) 짜지?"가 아니라 "무엇을(What) 만들지?"에 집중하는 흐름
- → 3장 도입부의 핵심 메시지("Claude는 How는 알지만 What은 모른다")와 정확히 같은 맥락

### 확장 기능 요약 (3.7장과 연결)
| 기능 | 역할 | 비유 |
|------|------|------|
| **Subagents** (`claude agents`) | 특정 작업 전담 보조 AI | 팀 안의 전문 담당자 |
| **Hooks** | 특정 이벤트(파일 수정 등)에서 자동 실행 | "이때는 무조건 이거 체크해" 규칙 |
| **MCP** | 외부 도구/데이터 연결 표준 프로토콜 | 외부 시스템 콘센트 |
| **Skills** | 재사용 가능한 작업 묶음 (`/슬래시 명령어`) | 즐겨찾기 매크로 |
| **Slash commands** | `/review`, `/init` 등 | 단축 명령 |

### Permission 모드 (책에 없지만 알아두면 좋음)
- `.claude/settings.json`의 `permissions.defaultMode`로 설정
- `auto` 모드: 안전한 편집/명령은 중단 없이 자동 실행 (편의 ↑, 검토 책임 ↑) 
- 책에서 강조한 "승인 전 실행" 흐름과 연결됨 (p.157) — 어떤 모드를 쓰느냐에 따라 워크플로가 달라짐

### 핵심 한 줄 정리 
> Claude Code는 "터미널 안에서 같이 일하는 AI 동료"다. 
> 그러나 **What을 명확히 정의하고, 작은 단위로 쪼개고, 검증 체계(Hook/테스트/Git)를 갖춰야** 비로소 잘 일하는 동료가 된다. 
> → 결국 3장 전체가 말하는 것: "AI는 도구가 아니라 팀원이다. 단, 팀원을 잘 쓰려면 매니지먼트가 필요하다."
